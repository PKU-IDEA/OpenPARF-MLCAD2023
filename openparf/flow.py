...
def place(params, pl_path):
    ...
    if params.benchmark_format == "bookshelf":
        ...
        with open(params.design_input, "r") as f:
            ...
            netlist_file = osp.join(osp.dirname(params.design_input), data["netlist"])
            place_file = osp.join(osp.dirname(params.design_input), data["place"])
        db.readFlexshelf(params.layout_input, netlist_file, place_file)
    elif params.benchmark_format == "ehbookshelf":
        assert (
            params.input_dir is not None
        ), "input_dir must be specified for ehbookshelf format"
        db.readEHBookshelf(params.input_dir)
    else:
        raise RuntimeError(
            "Unknown benchmark format: {}".format(params.benchmark_format)
        )
    logger.info("read benchmark takes %.3f seconds" % (time.time() - tt))
    ...
    place_params.cnp_utplace2_max_num_clock_pruning_per_iteration = (
        params.cnp_utplace2_max_num_clock_pruning_per_iteration
    )
    ...
    for at_id in range(len(place_params.at_name2id_map)):
        ...
        for resource_id in range(len(place_params.resource2area_types)):
            for value in place_params.resource2area_types[resource_id]:
                if value == at_id:
                    resource_name = (
                        db.layout().resourceMap().resource(resource_id).name()
                    )
    ...
    if params.macro_place_flag:
        place_engine.writeMacro(pl_path)
    else:
