...
class NesterovAcceleratedGradientOptimizer(Optimizer):
    ...
    def step(self, closure=None, cur_metric=None):
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group["params"]):
                ...
                while True:
                    ...
                    # logger.info("alpha_kp1 = %g, line_search_count = %d, obj_eval_count = %d" % (
                    #     alpha_kp1, backtrack_cnt, group['obj_eval_count']))
                    # logger.info("|g_k| = %.6E, |g_kp1| = %.6E" %
                    #             (g_k.norm(p=2), g_kp1.norm(p=2)))
                ...
                # if v_k.is_cuda:
                #    torch.cuda.synchronize()
                # logger.debug("\tline search %.3f ms" % ((time.time()-ttt)*1000))
        ...
        cur_metric.backtrack_cnt = backtrack_cnt
        return loss
