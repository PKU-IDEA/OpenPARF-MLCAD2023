...
class Placer(nn.Module):
    ...
    def _gp_adjust_area_condition(self, current_metric):
        ...
        max_overflow = None
        for at_name in self.params.gp_adjust_area_threshold_types:
        ...
        with DeferredAction() as defer:
            ...
            with self.adjust_area_stopwatch:
                ...
                if self.gp_adjust_resource_area:
                    ...
                    resource_overflow = self.get_utilization_map_overflow(
                        resource_opt_area
                    )
                    overflow_log_dict["Resource utilization overflow"] = "%6.2lf%%" % (
                        resource_overflow.item() * 100
                    )
                else:
                    overflow_log_dict["Resource utilization overflow"] = "None"
    ...
    def _ssir_legalization_condition(self, metrics):
        if (
            len(metrics) > 1
        ...
        ssir_legalization_threshold_type_ids = [
            self.placedb.getAreaTypeIndexFromName(x)
            for x in self.params.ssir_legalization_threshold_types
        ]
        ...
        if (
            ...
            and torch.all(
                current_metric.overflow[ssir_legalization_threshold_type_ids]
                < self.params.ssir_legalization_threshold
            )
            and torch.any(
                self.data_cls.total_movable_areas[self.data_cls.ssr_area_types] > 0
            )
    ...
    def io_legalization_condition(self, opt_iter, metrics):
        try:
            ...
            if max_io_grad > 1e2 * max_lut_ff_like_grad or len(metrics) > 150:
                return True
            else:
                return False
    ...
    def io_legalization(self, opt_iter):
        ...
        for io_at_id in io_at_ids:
            ...
            self.data_cls.area_type_mask[io_at_id] = 0
        # reset optimizer
        self.reset_optimizer(opt_iter)
    ...
    def forward(self):
        ...
        def constraint_fn(pos):
            ...
            if self.params.align_region_flag:
                self.op_cls.align_region_op(pos)
            if self.params.align_shape_flag:
                self.op_cls.align_shape_op(pos)
        ...
        # with open("bufg_net.txt") as f:
        #     lines = f.readlines()
        #     bufg_net_ids = torch.tensor([self.placedb.nameToNet(x.strip()) for x in lines]).to(self.device)
        #     print(self.data_cls.net_mask_ignore_large[bufg_net_ids])
        ...
        if self.params.global_place_flag:
            ...
            if self.params.plot_fgrain_flag:
                self.plot_fgrain(
                            osp.join(
                                self.params.plot_dir,
                                "fgrain_iter%s_initial.bmp"
                                % ("{:04}".format(opt_iter.iteration)),
                            ),
                            opt_iter,
                            self.data_cls.plot_group_types,
                            self.data_cls.plot_ginst_list,
                        )
            ...
            while opt_iter.iteration < 20 or not self.stop_condition(metrics):
                    ...
                    for opt_iter.iter_gamma in range(self.params.gamma_iters):
                        for opt_iter.iter_lambda in range(self.params.lambda_iters):
                            for opt_iter.iter_sub in range(self.params.sub_iters):
                                ...
                                if (
                                    self.params.plot_fgrain_flag
                                    and opt_iter.iteration
                                    % self.params.plot_iteration_frequency
                                    == 0
                                ):
                                    assert self.data_cls.plot_group_types is not None
                                    assert self.data_cls.plot_ginst_list is not None
                                    self.plot_fgrain(
                                        osp.join(
                                            self.params.plot_dir,
                                            "fgrain_iter%s.bmp"
                ...
                if self.io_legalization_condition(opt_iter, metrics):
                    ...
                    # with open("io_net.txt", "r") as f:
                    #     lines = f.readlines()
                    #     for line in lines:
                    #         a, b = line.strip().split()
                    #         a_id = self.placedb.nameToInst(a)
                    #         b_id = self.placedb.nameToInst(b)
                    #         print(f"{a}: {self.data_cls.io_pos_xyz[a_id][:2]}       {b}:{self.data_cls.pos[0][b_id]}")
                ...
                if self._ssir_legalization_condition(metrics):
                    if (
                    ...
                    if self.params.macro_place_flag:
                        # legalize the macro first
                        self.op_cls.ssr_mixed_size_legalize_op(self.data_cls.pos[0])
                        # then legalize the single instance
                        self.op_cls.region_mcf_lg_op(self.data_cls.pos[0])
                    else:
                    ...
                    if self.params.macro_place_flag:
                        inst_check_ids = (
                            self.op_cls.legality_macro_check_op.inst_check_ids
                        )
                        with torch.no_grad():
                            self.data_cls.inst_locs_xyz[
                                inst_check_ids, :2
                            ] = self.data_cls.pos[0][inst_check_ids]
                            self.data_cls.inst_locs_xyz[inst_check_ids, 2] = 0
                        legal = self.op_cls.legality_macro_check_op(
                            self.data_cls.inst_locs_xyz
                        )
                        if not legal:
                            logger.warning("Macro Placement is not LEGAL")
                        else:
                            logger.info("Macro placement is legal.")
                        return
    ...
    def initialize_params(self, eval_ops, opt_iter):
        """@brief initialize nonlinear placement parameters"""
        pos = self.data_cls.pos[0]

        # random initial placement
        if self.params.random_center_init_flag:
    ...
    def plot_fgrain(self, filename, opt_iter, group_types, ginst_list):
        draw_fgrain_place(
            self.data_cls.pos[0].data.cpu().numpy(),
            self.data_cls.inst_sizes_max.cpu().numpy(),
            self.data_cls.diearea,
    ...
    def writeMacro(self, filename):
        """@brief write to file"""
        self.apply()
        if not os.path.exists(os.path.dirname(filename)):
            os.makedirs(os.path.dirname(filename))
        self.placedb.writeMacroBookshelfPl(filename)
