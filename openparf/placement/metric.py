    ...
    def __str__(self):
        ...
        if self.hpwl is not None:
            content += ", HPWL %.3E (%g*%.3E + %g*%.3E)" % (
                ...
                self.params.wirelength_weights[0],
                self.hpwl[0],
                self.params.wirelength_weights[1],
                self.hpwl[1],
            )
        ...
        if self.grad_dicts is not None:
            d_o_w = (
                self.grad_dicts["density_grad_norm"]
                / self.grad_dicts["wirelength_grad_norm"]
            )
            if "fence_region_grad_norm" in self.grad_dicts:
                f_o_w = (
                    self.grad_dicts["fence_region_grad_norm"]
                    / self.grad_dicts["wirelength_grad_norm"]
                )
                content += (
                    ", nomalized-grad(wirelength/density/clock): %.3E/%.3E/%.3E"
                    % (1, d_o_w, f_o_w)
                )
            else:
                content += ", nomalized-grad(wirelength/density): %.3E/%.3E" % (
                    1,
                    d_o_w,
                )
        if self.backtrack_cnt is not None:
            content += ", backtrack count: {}".format(self.backtrack_cnt)
