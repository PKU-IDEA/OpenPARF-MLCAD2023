...
def draw_place(
    ...
    for area_type in target_area_types:
        ...
        img.strokeRect(
            layout_xl, layout_yl, layout_xh - layout_xl, layout_yh - layout_yl
        )
        ...
        if fixed_range and fixed_range[0] < fixed_range[1]:
            ...
            img.fillRects(
                locations[fixed_range[0] : fixed_range[1]],
        ...
        if filler_range and filler_range[0] < filler_range[1]:  # filler is included
            inst_ids = at_inst_ids[
                np.where(
                    np.logical_and(
                        at_inst_ids >= filler_range[0], at_inst_ids < filler_range[1]
                    )
                )
            ]
            img.setFillColor(
                color_map[area_type, 0],
                color_map[area_type, 1],
                color_map[area_type, 2],
                0.5,
            )
        ...
        if movable_range and movable_range[0] < movable_range[1]:
            inst_ids = at_inst_ids[
                np.where(
                    np.logical_and(
                        at_inst_ids >= movable_range[0], at_inst_ids < movable_range[1]
                    )
                )
            ]
            img.setFillColor(
    ...
    logging.info("plotting to %s takes %.3f seconds" % (filename, time.time() - tt))
    ...
    for inst_id in range(movable_range[0], movable_range[1]):
        ...
        if not (
            ...
            img.strokeLine(
                xy[0],
                xy[1],
                (cr_bbox[0] + cr_bbox[2]) / 2,
                (cr_bbox[1] + cr_bbox[3]) / 2,
            )
        else:
            in_cnt += 1
    if not os.path.exists(os.path.dirname(filename)):
...
def draw_fgrain_place(
    ...
    gtype_set = np.unique(group_types)
    assert len(gtype_set) <= len(color_map)
    for i, gid in enumerate(gtype_set):
        # create new iamge
        img = of.Image(width, height, layout_xl, layout_yl, layout_xh, layout_yh)
        ...
        # draw cells
        img.setFillColor(color_map[i, 0], color_map[i, 1], color_map[i, 2], 0.5)
        img.setStrokeColor(color_map[i, 0], color_map[i, 1], color_map[i, 2], 0.5)
        idx_list = np.squeeze((group_types == gid).nonzero())
        for gidx in idx_list:
            inst_ids = ginst_list[gidx]
            img.fillRects(locations[inst_ids], inst_sizes[inst_ids])
            # draw bounding box
            bbox_xl = np.min(locations[inst_ids, 0])
            bbox_yl = np.min(locations[inst_ids, 1])
            bbox_xh = np.max(locations[inst_ids, 0] + inst_sizes[inst_ids, 0])
            bbox_yh = np.max(locations[inst_ids, 1] + inst_sizes[inst_ids, 1])
            img.strokeRect(bbox_xl, bbox_yl, bbox_xh - bbox_xl, bbox_yh - bbox_yl)

        # write image
        if not os.path.exists(os.path.dirname(filename)):
            os.system("mkdir -p %s" % (os.path.dirname(filename)))
        draw_filename = filename.replace(".bmp", "_gid-{:02}.bmp".format(gid))
        img.end()
...
def draw_fence_regions(
    ...
    for fence_region_idx in target_fence_regions_indexes:
        ...
        movable_inst_ids = inst_ids[
            np.where(
                np.logical_and(
                    inst_ids >= movable_range[0], inst_ids < movable_range[1]
                )
            )
        ]
        ...
        fixed_inst_ids = inst_ids[
            np.where(
                np.logical_and(inst_ids >= fixed_range[0], inst_ids < fixed_range[1])
            )
        ]
        ...
        draw_filename = filename.replace(
            ".bmp", "_fr{:04}.bmp".format(fence_region_idx)
        )
        img.end()
        img.write(draw_filename)

    logging.info(
        "plotting to %s takes %.3f seconds"
