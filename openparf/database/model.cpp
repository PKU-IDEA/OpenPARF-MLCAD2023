...
container::ObserverPtr<const ModelPin> Model::modelPin(std::string const &name) const {
  auto ret = modelPinId(name);
  if (ret == std::numeric_limits<IndexType>::max()) {
    return {};
  } else {
...
container::ObserverPtr<Net> Model::getModelPinNet(ModelPin const &mp) {
  if (model_pin_to_net_->find(mp.id()) == model_pin_to_net_->end()) {
    ...
    return netlist_->net(model_pin_to_net_->at(mp.id()));
}
...
container::ObserverPtr<ModelPin> Model::getNetModelPin(Net const &n) {
  if (net_to_model_pin_->find(n.id()) == net_to_model_pin_->end()) {
    ...
    return container::ObserverPtr<ModelPin>(model_pins_.at(net_to_model_pin_->at(n.id())));
}
container::ObserverPtr<const ModelPin> Model::getNetModelPin(Net const &n) const {
  if (net_to_model_pin_->find(n.id()) == net_to_model_pin_->end()) {
    return container::ObserverPtr<const ModelPin>();
  } else
    return container::ObserverPtr<const ModelPin>(model_pins_.at(net_to_model_pin_->at(n.id())));
}
...
Model::IndexType Model::memory() const {
  IndexType ret = this->BaseType::memory() + sizeof(model_type_) + sizeof(model_pins_) + sizeof(name_) + sizeof(size_);
  for (auto const &mpin : model_pins_) {
    ret += mpin.memory();
  }
  ret += (model_pins_.capacity() - model_pins_.size()) * sizeof(decltype(model_pins_)::value_type);
