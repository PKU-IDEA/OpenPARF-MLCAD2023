...
bool PlaceDB::writeMacroBookshelfPl(std::string const &pl_file) const {
  return db_->writeMacroBookshelfPl(pl_file);
}
...
void PlaceDB::buildShape() {
  ...
  // assume flat netlist for now
  auto const &netlist          = top_module_inst->netlist();
  auto const &shape_constraint = db_->shapeConstraint();
  auto const &shapes           = shape_constraint.shapes();

  IndexType   num_shapes       = shapes.size();
  IndexType   num_insts        = netlist.numInsts();
  IndexType   num_shape_insts  = 0;
  for (auto &shape : shapes) {
    num_shape_insts += shape.instIds().size();
  }

  // build shape_insts_ and inst2shape_
  shape_insts_.reserve(num_shapes, num_shape_insts);
  inst2shape_.resize(num_insts, std::numeric_limits<IndexType>::max());
  IndexType count = 0;
  for (IndexType shape_id = 0; shape_id < num_shapes; ++shape_id) {
    auto const &shape = shapes[shape_id];
    shape_insts_.pushBackIndexBegin(count);
    for (auto const &old_inst_id : shape.instIds()) {
      IndexType new_inst_id = newInstId(old_inst_id);
      shape_insts_.pushBack(new_inst_id);
      inst2shape_.at(new_inst_id) = shape_id;
      ++count;
    }
  }
}

void PlaceDB::buildRegion() {
  auto const &design          = db_->design();
  auto const &layout          = db_->layout();
  auto        top_module_inst = design.topModuleInst();
  openparfAssert(top_module_inst);
  // assume flat netlist for now
  auto const &netlist           = top_module_inst->netlist();
  auto const &region_constraint = db_->regionConstraint();
  auto const &region_list       = region_constraint.region_list();

  IndexType   num_regions       = region_list.size();
  IndexType   num_insts         = netlist.numInsts();
  IndexType   num_region_insts  = 0;
  for (auto &region : region_list) {
    num_region_insts += region.instIds().size();
  }

  // build region_insts_ and inst2region_
  region_insts_.reserve(num_regions, num_region_insts);
  inst2region_.resize(num_insts, std::numeric_limits<IndexType>::max());
  IndexType count = 0;
  for (IndexType region_id = 0; region_id < num_regions; ++region_id) {
    auto const &region = region_list[region_id];
    region_insts_.pushBackIndexBegin(count);
    for (auto const &old_inst_id : region.instIds()) {
      IndexType new_inst_id = newInstId(old_inst_id);
      region_insts_.pushBack(new_inst_id);
      inst2region_.at(new_inst_id) = region_id;
      ++count;
    }
  }
}
...
std::vector<PlaceDB::BoxType> PlaceDB::collectRegionBoxes() const {
  auto const          &design            = db_->design();
  auto const          &layout            = db_->layout();
  auto const          &region_constraint = db_->regionConstraint();
  auto const          &region_list       = region_constraint.region_list();

  std::vector<BoxType> region_boxes;
  region_boxes.reserve(region_list.size());
  for (auto &region : region_list) {
    auto xl = region.bbox().xl();
    auto yl = region.bbox().yl();
    auto xh = region.bbox().xh();
    auto yh = region.bbox().yh();
    region_boxes.emplace_back(xl, yl, xh, yh);
  }

  return region_boxes;
}

std::vector<PlaceDB::CoordinateType> PlaceDB::collectFlattenRegionBoxes() const {
  std::vector<PlaceDB::BoxType> box_list = std::move(collectRegionBoxes());
  std::vector<CoordinateType>   flatten_list;
  for (auto const &box : box_list) {
    flatten_list.push_back(box.xl());
    flatten_list.push_back(box.yl());
