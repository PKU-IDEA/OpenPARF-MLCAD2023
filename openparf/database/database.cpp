 ...
 * @brief         :
 * -----
 * Last Modified: September 08 2023, 09:24:32, Friday
...
#include "io/ehbookshelf/ehbookshelf_driver.h"
...
class BookshelfDatabaseCallbacks : public bookshelfparser::BookshelfDatabase {
 public:
  ...
  /// @brief constructor
  explicit BookshelfDatabaseCallbacks(Database &db) : db_(db) {}
  ...
  void addShapeCbk(std::string const &str) {
    openparfAssertMsg(false, "shape constraint not supported yet\n");
    // ShapeModelType t = strToShapeModelType(str);
...
class EHBookshelfDatabaseCallbacks : public ehbookshelfparser::EHBookshelfDatabase {
 public:
  using IndexType = Object::IndexType;

  /// @brief constructor
  explicit EHBookshelfDatabaseCallbacks(Database &db) : db_(db) {}

  /// @brief set input dir
  void setInputDir(std::string const &input_dir) {
    input_dir_                    = input_dir;
    lib_file_                     = input_dir_ + "/" + "design.lib";
    scl_file_                     = input_dir_ + "/" + "design.scl";
    node_file_                    = input_dir_ + "/" + "design.nodes";
    pl_file_                      = input_dir_ + "/" + "design.pl";
    net_file_                     = input_dir_ + "/" + "design.nets";
    cascade_shape_file_           = input_dir_ + "/" + "design.cascade_shape";
    cascade_shape_instances_file_ = input_dir_ + "/" + "design.cascade_shape_instances";
    region_file_                  = input_dir_ + "/" + "design.regions";
    macros_file_                  = input_dir_ + "/" + "design.macros";
  }
  ...
  /// @brief getter for net file
  std::string const &netFile() const { return net_file_; }
  /// @brief getter for cascade shape file
  std::string const &cascadeShapeFile() const { return cascade_shape_file_; }
  /// @brief getter for cascade shape instances file
  std::string const &cascadeShapeInstancesFile() const { return cascade_shape_instances_file_; }
  /// @brief getter for region file
  std::string const &regionFile() const { return region_file_; }
  /// @brief getter for macros file
  std::string const &macrosFile() const { return macros_file_; }

  /* parsing lib file */
  void               addCellCbk(const std::string &str) {
    auto &design = db_.design();
    cur_model_   = &design.addModel(str);
    cur_model_->setModelType(ModelType::kCell);
  }
  void addCellInputPinCbk(const std::string &str) {
    auto &mpin = cur_model_->tryAddModelPin(str);
    mpin.setSignalDirect(SignalDirection::kInput);
    mpin.setSignalType(SignalType::kSignal);
  }
  void addCellOutputPinCbk(const std::string &str) {
    auto &mpin = cur_model_->tryAddModelPin(str);
    mpin.setSignalDirect(SignalDirection::kOutput);
    mpin.setSignalType(SignalType::kSignal);
  }
  void addCellClockPinCbk(const std::string &str) {
    auto &mpin = cur_model_->tryAddModelPin(str);
    mpin.setSignalDirect(SignalDirection::kInput);
    mpin.setSignalType(SignalType::kClock);
  }
  void addCellCtrlPinCbk(const std::string &str) {
    auto &mpin = cur_model_->tryAddModelPin(str);
    mpin.setSignalDirect(SignalDirection::kInput);
    mpin.setSignalType(ctrlPinNameToPinType(str));
  }
  void addCellCtrlSRPinCbk(const std::string &str) {
    openparfAssertMsg(false, "EHBookshelfDatabaseCallbacks::addCellCtrlSRPinCbk not implemented yet");
  }
  void addCellCtrlCEPinCbk(const std::string &str) {
    openparfAssertMsg(false, "EHBookshelfDatabaseCallbacks::addCellCtrlCEPinCbk not implemented yet");
  }
  void addCellInputCasPinCbk(const std::string &str) {
    openparfAssertMsg(false, "EHBookshelfDatabaseCallbacks::addCellInputCasPinCbk not implemented yet");
    /* do nothing */
  }
  void addCellOutputCasPinCbk(const std::string &str) {
    openparfAssertMsg(false, "EHBookshelfDatabaseCallbacks::addCellOutputCasPinCbk not implemented yet");
  }
  void addCellParameterCbk(const std::string &str) {
    openparfAssertMsg(false, "EHBookshelfDatabaseCallbacks::addCellParameterCbk not implemented yet");
  }
  ...
  void endSiteMapCbk() {
    if (last_site_) {
      // FIXME(Jing Mai): this is a hack to make the last SLICE site to have height 1
      if (last_site_->siteTypeId() == db_.layout().siteTypeMap().siteTypeId("SLICE")) {
        last_site_->setBbox({last_site_->siteMapId().x(), last_site_->siteMapId().y(), last_site_->siteMapId().x() + 1,
                last_site_->siteMapId().y() + 1});
      } else {
        last_site_->setBbox({last_site_->siteMapId().x(), last_site_->siteMapId().y(), last_site_->siteMapId().x() + 1,
                db_.layout().siteMap().height()});
      }
    }
  }
  void setSiteMapEntryCbk(unsigned x, unsigned y, std::string const &site_type) {
    ...
    if (last_site_) {
      if (last_site_->siteMapId().x() == site.siteMapId().x()) {
        ...
        // FIXME(Jing Mai): this is a hack to make the SLICE site to have height 1
        if (last_site_->siteTypeId() == db_.layout().siteTypeMap().siteTypeId("SLICE")) {
          last_site_->setBbox({last_site_->siteMapId().x(), last_site_->siteMapId().y(),
                  last_site_->siteMapId().x() + 1, last_site_->siteMapId().y() + 1});
        } else {
          last_site_->setBbox({last_site_->siteMapId().x(), last_site_->siteMapId().y(),
                  last_site_->siteMapId().x() + 1, db_.layout().siteMap().height()});
        }
      }
    }
    last_site_ = &site;
  ...
  void initClockRegionsCbk(unsigned width, unsigned height) {
    openparfAssertMsg(false, "EHBookshelfDatabaseCallbacks::initClockRegionsCbk not implemented yet");
  }
  void addClockRegionCbk(const std::string &name,
          unsigned                          xlo,
          unsigned                          ylo,
          unsigned                          xhi,
          unsigned                          yhi,
          unsigned                          ymid,
          unsigned                          hcxmin) {
    openparfAssertMsg(false, "EHBookshelfDatabaseCallbacks::addClockRegionCbk not implemented yet");
  }

  /* parsing nodes file */
  void addNodeCbk(const std::string &node_name, const std::string &cell_name) {
    if (!top_model_) {
      top_model_ = &db_.design().addModel("EHBookshelf.TOP");
      top_model_->setModelType(ModelType::kModule);
      db_.design().setTopModuleInstId(top_model_->id());
    }
    auto model_id = db_.design().modelId(cell_name);
    // NOTE: We DO NOT support URAM node in the current implementation.
    if (cell_name.find("URAM") != std::string::npos) {
      openparfPrint(kError, "openparf DO NOT support URAM node in the current implementation\n");
    }
    if (model_id < db_.design().numModels()) {
      auto &model = db_.design().model(model_id);
      auto &inst  = top_model_->tryAddInst(node_name);
  ...
  /* parsing pl file */
  void setFixedNodeCbk(const std::string &node_name, unsigned x, unsigned y, unsigned z) {
    auto inst_proxy = top_model_->inst(node_name);
    if (inst_proxy) {
      auto &inst = *inst_proxy;
      inst.attr().setLoc({x, y, z});
      ...
      openparfPrint(kError, "instance %s not found, ignored\n", node_name.c_str());
    }
  }

  /* parsing nets file */
  void addNetCbk(const std::string &net_name, unsigned degree) {
    auto &net = top_model_->tryAddNet(net_name);
    if (net.id() + 1 != top_model_->netlist()->numNets()) {   // not newly added
      openparfPrint(kWarn, "net %s with degree %u already defined in model %s\n", net_name.c_str(), degree,
              top_model_->name().c_str());
  ...
  void addPinCbk(const std::string &node_name, const std::string &cell_pin_name) {
    auto inst_proxy = top_model_->inst(node_name);
    if (!inst_proxy) {
      openparfPrint(kWarn, "net %s instance %s not defined, ignored\n",
              top_model_->netlist()->net(cur_net_->id()).attr().name().c_str(), node_name.c_str());
  ...
  void endNetCbk() { /* do nothing */
  }

  /* parsing cascade shape file */
  void addCascadeShapeCbk(const std::string           &shape_model_name,
          const std::pair<int32_t, int32_t>            shape_model_size,
          const std::vector<std::vector<std::string>> &element_list) {
    using PointType                   = ShapeModel::PointType;
    Design          &design           = db_.design();
    ShapeConstraint &shape_constraint = db_.shapeConstraint();
    ShapeModel      &shape_model      = shape_constraint.tryAddShapeModel(shape_model_name);

    if (shape_model.id() + 1 != shape_constraint.numShapeModels()) {   // not newly added
      openparfPrint(kWarn, "shape model %s with size %d %d already defined\n", shape_model_name.c_str(),
              shape_model_size.first, shape_model_size.second);
      return;
    }
    shape_model.setType(strToShapeModelType(shape_model_name));
    shape_model.setName(shape_model_name);
    openparfAssertMsg(shape_model_size.first == element_list.size(), "shape model size mismatch\n");
    // shape_model_size.first: y-axis
    // shape_model_size.second: x-axis
    for (IndexType ix = 0; ix < shape_model_size.second; ++ix) {
      for (IndexType iy = 0; iy < shape_model_size.first; ++iy) {
        openparfAssertMsg(shape_model_size.second == element_list[iy].size(), "shape model size mismatch\n");
        IndexType     model_id      = design.modelId(element_list[iy][ix]);
        PointType     offset        = {ix, iy};
        ShapeElement &shape_element = shape_model.addShapeElement(model_id, offset);
      }
    }
  }
  ...
  void addCascadeShapeInstanceCbk(const std::string &shape_model_name,
          const std::string                          shape_instance_name,
          const std::vector<std::string>             shape_instance_list) {
    ShapeConstraint &shape_constraint = db_.shapeConstraint();
    /* walk around to handle the benchmark bug in MLCAD 2023 */ {
      for (const std::string &cell_name : shape_instance_list) {
        auto inst_proxy = top_model_->inst(cell_name);
        if (!inst_proxy) {
          openparfPrint(kWarn, "shape %s instance %s not defined, ignored this shape\n", shape_instance_name.c_str(),
                  cell_name.c_str());
          return;
        }
      }
    }
    Shape         &shape        = shape_constraint.tryAddShape(shape_instance_name);
    ShapeModelMap &shape_models = shape_constraint.shapeModels();
    if (shape.id() + 1 != shape_constraint.numShapes()) {   // not newly added
      openparfPrint(kWarn, "shape %s already defined\n", shape_instance_name.c_str());
      return;
    }
    openparfAssertMsg(shape_models.ifExist(shape_model_name), "shape model %s not defined\n", shape_model_name.c_str());
    shape.setShapeModelId(shape_constraint.shapeModelId(shape_model_name));
    shape.setName(shape_instance_name);
    for (const std::string &cell_name : shape_instance_list) {
      auto inst_proxy = top_model_->inst(cell_name);
      if (!inst_proxy) {
        openparfPrint(kWarn, "shape %s instance %s not defined, ignored\n", shape_instance_name.c_str(),
                cell_name.c_str());
        return;
      }
      auto     &inst    = *inst_proxy;
      IndexType inst_id = inst.id();
      openparfAssertMsg(inst_id != Object::invalidIndex, "instance %s not defined\n", cell_name.c_str());
      shape.addinstId(inst_id);
    }
  }

  /* parsing region file */
  void addRegionConstraintCbk(std::string const &region_name, int32_t num_boxes, const std::vector<Box> &boxes) {
    openparfAssertMsg(num_boxes == boxes.size(), "region constraint size mismatch\n");
    openparfAssertMsg(num_boxes == 1, "only support one region boxes\n");
    RegionBoxType box_type = strToRegionBoxType(boxes.at(0).box_type);
    openparfAssertMsg(box_type == RegionBoxType::kRect, "only support rect region box\n");
    RegionConstraint &region_constraint = db_.regionConstraint();
    Region           &region            = region_constraint.tryAddRegion(region_name);
    if (region.id() + 1 != region_constraint.numRegions()) {   // not newly added
      openparfPrint(kWarn, "region %s already defined\n", region_name.c_str());
      return;
    }
    region.setName(region_name);
    Region::BoxType bbox = {static_cast<IndexType>(boxes.at(0).xlo), static_cast<IndexType>(boxes.at(0).ylo),
            static_cast<IndexType>(boxes.at(0).xhi), static_cast<IndexType>(boxes.at(0).yhi)};
    region.setBbox(bbox);
  }

  void addRegionMappingCbk(std::string const &cell_name, std::string const &region_name) {
    RegionConstraint &region_constraint = db_.regionConstraint();
    Design           &design            = db_.design();
    auto              inst_proxy        = top_model_->inst(cell_name);
    if (!inst_proxy) {
      openparfPrint(kWarn, "region %s instance %s not defined, ignored\n", region_name.c_str(), cell_name.c_str());
      return;
    }
    auto     &inst    = *inst_proxy;
    IndexType inst_id = inst.id();
    openparfAssertMsg(inst_id != Object::invalidIndex, "instance %s not defined\n", cell_name.c_str());
    region_constraint.tryAddMapping(inst_id, region_name);
  }
  ...
  void addMacroInstCbk(std::string const &node_name) {
    ...
    auto &inst = *inst_proxy;
    db_.addInstMacroId(inst.id());
    // openparfDebugMileStoneMsg("Macro instance %s (old id) %d", node_name.c_str(), inst.id());
  }

 protected:
  Database   &db_;   ///< reference to a Database object

  /// ehbookshelf files and path
  std::string input_dir_;                      ///< input file directory
  std::string lib_file_;                       ///< base .lib file name
  std::string scl_file_;                       ///< base .scl file name
  std::string node_file_;                      ///< base .node file name
  std::string pl_file_;                        ///< base .pl file name
  std::string net_file_;                       ///< base .net file name
  std::string cascade_shape_file_;             ///< base .cascade_shape file name
  std::string cascade_shape_instances_file_;   ///< base .cascade_shape_instances file name
  std::string region_file_;                    ///< base .region file name
  std::string macros_file_;                    /// base .macros file name
...
class VerilogDatabaseCallbacks : public VerilogParser::VerilogDataBase {
 public:
  explicit VerilogDatabaseCallbacks(Database &db) : db_(db) {
...
void Database::readEHBookshelf(std::string const &input_dir) {
  EHBookshelfDatabaseCallbacks cbk(*this);
  cbk.setInputDir(input_dir);
  ehbookshelfparser::Driver driver(cbk);
  std::ifstream             ifs;

  openparfPrint(kInfo, "Parsing enhanced bookshelf bnechmark %s\n", input_dir.c_str());
  for (auto file : {cbk.libFile(), cbk.sclFile(), cbk.nodeFile(), cbk.plFile(), cbk.netFile(), cbk.regionFile(),
               cbk.cascadeShapeFile(), cbk.cascadeShapeInstancesFile(), cbk.macrosFile()}) {
    if (!file.empty()) {
      Stopwatch timer;
      timer.start();
      openparfPrint(kInfo, "Parsing file %s\n", file.c_str());
      ifs.open(file);
      if (!ifs.good()) {
        // cascadeShapeInstanceFile is optional, so we don't need to report error
        if (file == cbk.cascadeShapeInstancesFile()) {
          openparfPrint(kWarn, "File %s does NOT exist, ignored\n", file.c_str());
          continue;
        } else {
          openparfAssertMsg(ifs.good(), "Cannot open file %s", file.c_str());
        }
      } else {
        driver.parse_stream(ifs);
        ifs.close();
...
bool Database::writeMacroBookshelfPl(std::string const &pl_file) {
  openparfPrint(kInfo, "write to %s\n", pl_file.c_str());
  std::ofstream ofs(pl_file.c_str());
  if (!ofs.good()) {
    openparfPrint(kError, "failed to open file %s for write", pl_file.c_str());
  ...
  std::vector<IndexType> inst_pl_ids;
  auto                  &shapes = shape_constraint_.shapes();
  std::transform(shapes.begin(), shapes.end(), std::back_inserter(inst_pl_ids),
          [](auto &shape) { return shape.instId(0); });

  shape_constraint_.buildInstShapeIdsSet();
  for (auto const &inst_id : inst_macro_ids_) {
    if (!shape_constraint_.hasInst(inst_id)) {
      inst_pl_ids.push_back(inst_id);
    }
  }

  for (auto const &inst_id : inst_pl_ids) {
    auto const &inst = netlist.inst(inst_id);
    IndexType   id1d = valid_site_map.at(inst.attr().loc().x() * site_map.height() + inst.attr().loc().y());
    openparfAssertMsg(id1d != std::numeric_limits<IndexType>::max(),
            "Instance %s(%d, %d) is not located in any vaild site.", inst.attr().name().c_str(), inst.attr().loc().x(),
...
void Database::copy(Database const &rhs) {
  ...
  design_           = rhs.design_;
  layout_           = rhs.layout_;
  shape_constraint_ = rhs.shape_constraint_;
  params_           = rhs.params_;
}
...
void Database::move(Database &&rhs) {
  ...
  shape_constraint_ = std::move(rhs.shape_constraint_);
  params_           = std::move(rhs.params_);
}

Database::IndexType Database::memory() const {
  IndexType ret =
          this->BaseType::memory() + design_.memory() + layout_.memory() + shape_constraint_.memory() + sizeof(params_);
  return ret;
}
...
std::ostream &operator<<(std::ostream &os, Database const &rhs) {
  ...
  os << ", \nshape_constraint_ : " << rhs.shape_constraint_ << std::endl;
