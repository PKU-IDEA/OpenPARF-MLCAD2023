...
class PlaceDB {
 public:
  ...
  IndexType                     clockIdToNetId(IndexType clockId) const {
    openparfAssert(clockId >= 0 && clockId < num_clock_nets_);
  ...
  IndexType netIdToClockId(IndexType netId) const {
    openparfAssert(netId >= 0 && netId < numNets());
  ...
  /// @brief getter for shape_insts_
  container::FlatNestedVector<IndexType, IndexType> const &shapeInsts() const { return shape_insts_; }

  /// @brief getter for inst2shape_
  std::vector<IndexType> const                            &inst2Shape() const { return inst2shape_; }

  /// @brief getter for region_insts_
  container::FlatNestedVector<IndexType, IndexType> const &regionInsts() const { return region_insts_; }

  /// @brief getter for inst2region_
  std::vector<IndexType> const                            &inst2Region() const { return inst2region_; }


  /// @brief all site boxes
  std::vector<BoxType>                                     collectRegionBoxes() const;

  /// @brief collect sites boxes and return as a flatten list
  std::vector<CoordinateType>                              collectFlattenRegionBoxes() const;

  // =============================================
  // [BEGIN ] >>>>>>> CARRY  Attribute >>>>>>>
  // =============================================
  ...
  /// brief wirte bookshelf .pl file for macros, specialized for MLCAD 2023 FPGA macro placement contest
  bool                                                     writeMacroBookshelfPl(std::string const &pl_file) const;
 ...
 protected:
  ...
  void                                              buildNetlist();
  /// @brief build shape and form flat data structure
  void                                              buildShape();
  /// @brief build region and form flat data structure
  void                                              buildRegion();
  ...
  /// shape information in flat storage
  container::FlatNestedVector<IndexType, IndexType> shape_insts_;   ///< instances of each shape
  std::vector<IndexType>
          inst2shape_;   ///< map inst to its parent shape, std::numeric_limits<IndexType>::max() if not in a shape

  /// region information in flat storage
  container::FlatNestedVector<IndexType, IndexType> region_insts_;   ///< instances of each region
  std::vector<IndexType>
          inst2region_;   ///< map inst to its parent region, std::numeric_limits<IndexType>::max() if not in a region


  // =============================================
  // [BEGIN ] >>>>>>> CARRYShape Attribute >>>>>>>
  // =============================================

  container::FlatNestedVector<IndexType, IndexType> chain_insts_;   ///< the new inst Id of each chain

  // =============================================
  // [  END] <<<<<<<< CARRYShape Attribute <<<<<<<
  // =============================================
};
