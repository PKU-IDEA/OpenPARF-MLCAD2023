...
class FlatNestedVector {
 public:
  ...
  class ConstVectorProxy {
   public:
    /// @brief constructor
    ConstVectorProxy(ConstIteratorType b, ConstIteratorType e) : begin_(b), end_(e) {}
  ...
  /// @brief constructor
  explicit FlatNestedVector(std::vector<std::vector<ValueType>> const &rhs) {
  ...
  ConstVectorProxy at(IndexType i) const {
    return ConstVectorProxy(data_.begin() + indexBeginAt(i), data_.begin() + indexEndAt(i));
  }

  /// @brief getter the range of data at the first dimension
  VectorProxy at(IndexType i) {
    openparfAssert(i < size1());
    return VectorProxy(data_.begin() + indexBeginAt(i), data_.begin() + indexEndAt(i));
  }
  ...
  ValueType                    &at(IndexType i, IndexType j) {
    openparfAssertMsg(j < size2(i), "j = %lu, size2(%lu) = %lu", j, i, size2(i));
    return data_[indexBeginAt(i) + j];
  }
  ...
  IndexType memory() const {
    return sizeof(data_) + sizeof(dim2_start_) + data_.capacity() * sizeof(ValueType) +
 ...
 protected:
  ...
  friend std::ostream &operator<<(std::ostream &os, FlatNestedVector const &rhs) {
    os << "FlatNestedVector"
       << "("
       << "[" << rhs.size() << " entries, " << rhs.size1() << " rows]";
    for (IndexType i = 1; i < (IndexType) rhs.dim2_start_.size(); ++i) {
      os << " [" << rhs.size2(i - 1) << "][";
      for (IndexType j = rhs.dim2_start_[i - 1], je = rhs.dim2_start_[i]; j < je; ++j) {
        os << " " << rhs.data_[j];
      }
      os << " ]";
    }
  ...
  std::vector<IndexType> dim2_start_;   ///< length of (number of size1 + 1),
...
#endif   // OPENPARF_CONTAINERS_FLAT_NESTED_VECTOR_HPP_
