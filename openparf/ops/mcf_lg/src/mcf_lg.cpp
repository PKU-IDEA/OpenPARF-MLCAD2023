 ...
 * @brief         :
 * -----
 * Last Modified: July 23 2023, 14:41:19, Sunday
...
template<typename T>
void MinCostFlowLegalizer<T>::add_sssir_instances(at::Tensor inst_ids,
...
at::Tensor MinCostFlowLegalizer<T>::forward(at::Tensor pos) {
  ...
  while (!stop_flag) {
    ...
    for (int idx = 0; idx < num_models; idx++) {
      ...
      openparfPrint(MessageType::kDebug, "Iter %i model %i: check sites with distance smaller than %f\n", iter, idx,
              max_dis);
      ...
      for (int32_t i = 0; i < m.num_insts; i++) {
        ...
        for (int32_t s = 0; s < m.num_sites; s++) {
          ...
          if (m_dist >= min_dis && m_dist < max_dis &&
                  (!honor_clock_region_constraints || inst_clock_legal_at_site(inst_id, xl, yl))) {
            ...
            mcf_problem.capacity_lower_bound[arc] = 0;
            mcf_problem.capacity_upper_bound[arc] = 1;
            if (honor_clock_region_constraints) m.hc_arcs.at(hc_id).push_back(m.inst_to_site_arcs.size() - 1);
          }
        }
      }
    }
    ...
    for (auto &m : sssir_model_infos) {
      ...
      if (total_flow != m.num_insts) {
        openparfPrint(MessageType::kDebug, "Total flow %i not equal to num of inst %i for model %i\n", total_flow,
                m.num_insts, m.model_id);
        stop_flag = false;
        mcf_legal = false;
      }
    }
...
template<typename T>
bool MinCostFlowLegalizer<T>::inst_clock_legal_at_site(int32_t inst_id, int32_t site_x, int32_t site_y) {
...
template<typename T>
std::vector<std::pair<int32_t, int32_t>> MinCostFlowLegalizer<T>::check_half_column_sat_and_prune_clock() {
...
#define INSTANTIATE_PYTHON_CLASS(T)                                                                                    \
  py::class_<OPENPARF_NAMESPACE::MinCostFlowLegalizer<T>>(m, "MinCostFlowLegalizer_" #T)                               \
          .def(py::init<OPENPARF_NAMESPACE::database::PlaceDB const &>())                                              \
          .def("add_sssir_instances", &OPENPARF_NAMESPACE::MinCostFlowLegalizer<T>::add_sssir_instances,               \
                  "Add SSSIR instances")                                                                               \
          .def("forward", &OPENPARF_NAMESPACE::MinCostFlowLegalizer<T>::forward, "Min-cost flow legalization forward") \
          .def("set_honor_clock_constraints",                                                                          \
                  &OPENPARF_NAMESPACE::MinCostFlowLegalizer<T>::set_honor_clock_constraints,                           \
                  "Set whether to consider clock region constraints")                                                  \
          .def("set_max_clk_per_half_column",                                                                          \
                  &OPENPARF_NAMESPACE::MinCostFlowLegalizer<T>::set_max_clk_per_half_column,                           \
                  "Set maximum number of clock nets in each half column")                                              \
          .def("reset_clock_available_clock_region",                                                                   \
                  &OPENPARF_NAMESPACE::MinCostFlowLegalizer<T>::reset_clock_available_clock_region,                    \
