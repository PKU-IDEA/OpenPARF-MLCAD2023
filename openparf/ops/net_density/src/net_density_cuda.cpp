...
void    net_density_forward(at::Tensor pin_pos,
     ...
     OPENPARF_DISPATCH_FLOATING_TYPES(pin_pos, "NetDensityCudaLauncher", [&] {
    NetDensityCudaLauncher<scalar_t>(OPENPARF_TENSOR_DATA_PTR(pin_pos, scalar_t),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &OPENPARF_NAMESPACE::net_density_forward, "compute net density map (CUDA)");
}
