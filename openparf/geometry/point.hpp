...
class BasePoint : public std::array<CoordinateType, DimensionCount> {
 public:
  ...
  BasePoint() : BaseType() { this->fill(std::numeric_limits<CoordinateType>::max()); }
  /// @brief constructor
  template<class... Args>
  BasePoint(CoordinateType const &first, Args... args) : BaseType({first, args...}) {}
...
class Point<CoordinateType, 2> : public BasePoint<CoordinateType, 2> {
 public:
  ...
  Point left() const {
    static_assert(std::is_integral<CoordinateType>::value, "Integral type is required.");
    return Point(this->get<0>() - 1, this->get<1>());
  }
  Point right() const {
    static_assert(std::is_integral<CoordinateType>::value, "Integral type is required.");
    return Point(this->get<0>() + 1, this->get<1>());
  ...
  Point bottom() const {
    static_assert(std::is_integral<CoordinateType>::value, "Integral type is required.");
    return Point(this->get<0>(), this->get<1>() - 1);
  }
  Point top() const {
    static_assert(std::is_integral<CoordinateType>::value, "Integral type is required.");
    return Point(this->get<0>(), this->get<1>() + 1);
  ...
  CoordinateType manhattanDistance(Point const &rhs) const { return std::abs(x() - rhs.x()) + std::abs(y() - rhs.y()); }
  ...
  friend Point operator+(Point const &v1, Point const &v2) { return Point(v1.x() + v2.x(), v1.y() + v2.y()); }
  /// @brief overalod operator-
  friend Point operator-(Point const &v1, Point const &v2) { return Point(v1.x() - v2.x(), v1.y() - v2.y()); }
};
...
class Point<CoordinateType, 3> : public BasePoint<CoordinateType, 3> {
 public:
  ...
  void set(CoordinateType const &v0, CoordinateType const &v1, CoordinateType const &v2) {
    this->setX(v0);
    this->setY(v1);
    this->setZ(v2);
  }
  ...
  /// @brief overload output stream
  friend std::ostream &operator<<(std::ostream &os, Point const &rhs) {
    os << className<decltype(rhs)>() << "(" << rhs.x() << ", " << rhs.y() << ", " << rhs.z() << ")";
    return os;
  }
  /// @brief overload input stream
  friend std::istream &operator>>(std::istream &is, Point &rhs) {
...
//////// for point ////////
template<typename CoordinateType, std::size_t DimensionCount>
struct tag<OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount>> {
  typedef point_tag type;
};

template<typename CoordinateType, std::size_t DimensionCount>
struct point_type<OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount>> {
  typedef typename OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount> type;
};

template<typename CoordinateType, std::size_t DimensionCount>
struct coordinate_type<OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount>> {
  typedef CoordinateType type;
};

template<typename CoordinateType, std::size_t DimensionCount>
struct coordinate_system<OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount>>
    : public coordinate_system<model::point<CoordinateType, DimensionCount, cs::cartesian>> {};

template<typename CoordinateType, std::size_t DimensionCount>
struct dimension<OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount>>
    : public dimension<model::point<CoordinateType, DimensionCount, cs::cartesian>> {};

template<typename CoordinateType, std::size_t DimensionCount, std::size_t Dimension>
struct access<OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount>, Dimension> {
  static inline CoordinateType get(OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount> const &p) {
    return p.template get<Dimension>();
  }

  static inline void set(OPENPARF_NAMESPACE::geometry::Point<CoordinateType, DimensionCount> &p,
