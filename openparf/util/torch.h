...
#define CHECK_FLAT_CPU(x)     AT_ASSERTM(!x.is_cuda(), #x " must be a flat tensor on CPU")
#define CHECK_FLAT_CUDA(x)    AT_ASSERTM(x.is_cuda(), #x " must be a flat tensor on GPU")
#define CHECK_EVEN(x)         AT_ASSERTM((x.numel() & 1) == 0, #x " must have even number of elements")
#define CHECK_DIVISIBLE(x, y) AT_ASSERTM((x.numel() % y) == 0, #x " must have " #y "-divisible number of elements")
#define CHECK_CONTIGUOUS(x)   AT_ASSERTM(x.is_contiguous(), #x " must be contiguous")
#define CHECK_TYPE(x, y)      AT_ASSERTM(x.dtype() == y, #x " must be a " #y " tensor")
